...
set.seed(123)
for (i in 1:I){
  treat[[i]] <- rnorm(n1, 110, 10)
  control[[i]] <- rnorm(n0, 100, 10)
  ...
  cov_dl[i] <- ifelse(ci_dr[[i]][1] <= 2.93 & ci_dr[[i]][2] >= 2.93, 1, 0)
...
write.csv(output, "F:/Output/Setup 1/Para_Spec/Semiparametric/PE/T3C_800.csv")
