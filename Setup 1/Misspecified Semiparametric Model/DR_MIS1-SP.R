...
#'                Delta based CI for Setup - I, when the semiparametric model is misspecified.
...
cov_dl <- numeric(I)
SP_var2 <- numeric(I)
output <- list(mode="vector",length=I)

set.seed(123)
for (i in 1:I){
  treat[[i]] <- rrayleigh(n1, scale = 200)
  control[[i]] <- rrayleigh(n0, scale = 110)
  ...
  ## Coverage
  cov_dl[i] <- ifelse(ci_dr[[i]][1] <= 4.53 & ci_dr[[i]][2] >= 4.53, 1, 0)
...
mean(cov_dl)                                      # Coverage prob of delta-based CI
...
write.csv(output, "F:/Output/Setup 1/SP_MIS/Try_1205/SP/PE/T3C_800.csv")
