...
for (i in 1:I){
  ...
  #Coverage
  c1[i] <- ifelse(ci.d.mle[[i]][1] <= 2.58 & ci.d.mle[[i]][2] >= 2.58, 1, 0)
  c2[i] <- ifelse(ci.bs[[i]][1] <= 2.58 & ci.bs[[i]][2] >= 2.58, 1, 0)
...
mean(c1)                 # Coverage prob of delta-based CI
mean(c2)                 # Coverage prob of BS based CI

### output for boxplot##

output <- cbind(n_t,n_c,diff1,diff2)
write.csv(output, "F:/Output/Setup 1/Misspecified Para/Parametric/CI/T3C_CI_800.csv")
