...
#'                for Setup - II, when the semiparametric model is misspecified.
...
simudata <- function(shape_t = 1.2, shape_c = 0.5, rate_t = 1.5, rate_c = 1.5, n0=50,n1=50){
...
btsimu<-function(I=1000,J=1000,shape_t = 1.2, shape_c = 0.5, rate_t = 1.5, rate_c = 1.5, n0=50,n1=50,seed1=123){
...
cov <- ifelse(results[,1] <= 5.47 & results[,2] >= 5.47,1,0)
median(results[,3])      # median of BS difference
mean(cov)                    # Coverage of BS based CI

### output for boxplot##
output <- cbind(n_1,n_0,bs_diff_dr)
write.csv(output, "F:/Output/Setup 2/SP_MIS/Try 2/SemiParametric/BS-CI/C3T_CI_400.csv")
