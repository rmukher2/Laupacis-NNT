...
#' @Description - Obtain the Parametric CIs of Laupacis' NNT for Setup - II, when the parametric model
...
tau = log(3)
 p_c1   = function(data, indices) {
   p.c     = ifelse( mean( data[indices] > tau, na.rm = T ) < 0.001, # prob of success in the control group
                     0.001,
 ...
 p_t1 = function(data, indices) {
   p.t     = ifelse( mean( data[indices] > tau, na.rm = T ) > 0.999, # prob of success in the treatment group
                     0.999,
                     mean( data[indices] > tau, na.rm = T  ) )
...
#coverage
c1[i] <- ifelse(ci.d.mle[[i]][1] <= 3.21 & ci.d.mle[[i]][2] >= 3.21, 1, 0)
c2[i] <- ifelse(ci.bs[[i]][1] <= 3.21 & ci.bs[[i]][2] >= 3.21, 1, 0)
...
median(diff1[diff1 != 'NaN'])                 # median of delta difference
median(diff1)
median(diff2)                                 # median of BS difference

mean(c1)
mean(c1, na.rm = TRUE)                        # Coverage of delta-based CI
mean(c2)                                      # Coverage of BS CI

### output for boxplot##
output <- cbind(n_t,n_c,diff1,diff2)
write.csv(output, "F:/Output/Setup 2/Para_Spec/Parametric/CI/C3T_CI_800.csv")
