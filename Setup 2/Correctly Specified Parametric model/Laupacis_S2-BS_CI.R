...
#' @Description - Obtain the BS CI for Nonparametric Laupacis NNT for Setup - II, when the parametric model
...
## Coverage
coverage[i] <- ifelse(ci_bs[[i]][1] <= 3.21 & ci_bs[[i]][2] >= 3.21, 1, 0)
}
median(bs_diff)             # median of BS difference
mean(coverage)                # Coverage prob of BS based CI

### output for boxplot##
output <- cbind(n_t,n_c,bs_diff)
write.csv(output, "F:/Output/Setup 2/Para_Spec/NonParametric/BS - CI/C3T_BS-CI_800.csv")
